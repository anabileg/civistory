...
// القائمة الرئيسية - تم الربط من العنصر الثالث بـ 10 أقسام ثابتة
const navLinks = document.getElementById('navLinks');
if (navLinks && data.nav && Array.isArray(data.nav)) {
    const sectionIds = ['sec_goals', 'sec_shout', 'sec_awareness', 'sec_media', 'sec_stats', 'sec_losses', 'sec_programs', 'sec_vision', 'sec_reference', 'sec_partners'];
    navLinks.innerHTML = data.nav.map((item, index) => {
        // إذا كان الفهرس 2 أو أكبر (أي العنصر الثالث وما بعده) وضمن نطاق الأقسام العشرة
        if (index >= 2 && index < 12) {
            const sectionId = sectionIds[index - 2];
            return `<a href="#${sectionId}">${item}</a>`;
        }
        // البقية (الرئيسية ومن نحن) تبقى بدون ربط داخلي كما طلبتم
